...
{    
    ...
    gestImage->chargerImagesMap(largeur*hauteur, _numsTuiles, _numsElements, _cheminsTuiles, _cheminsElements);
    ...
    if (elemsON && statut == CIBLAGE)
    {
        ...
        for(int numCase=0; numCase<hauteur*largeur; numCase++)
        {
            ...
            if (numElemAct > 0)
            {
                ...
                GL_DessinElement(inclinaisonElements);
                glPopName(); glPopName();
